...
use crate::{
    ...
    pages::{
        dashboard_page, render_keys_page, render_request_detail_page, render_requests_page,
        render_upstream_secrets_page,
    },
    ...
    match read_payload(&state.config.payload_dir, &state.master_key, path) {
        Ok(bytes) => {
            ...
            response.headers_mut().insert(
                ...
                HeaderValue::from_str(&format!("attachment; filename=\"{}_{}.json\"", id, kind))
